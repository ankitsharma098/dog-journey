...
- `Dog Journey Royal Redesign.html` — an interactive HTML prototype. Open it in a browser; the chips under the first phone jump between all 12 screens. Treat it as the source of truth for look and behaviour.
...
3. **Home dashboard** — a new screen and route; it becomes tab 1. See "Home dashboard" in the README. Reads from `PetsBloc`, the reminders table (`db-design/06_billing_reminders.sql`) and `NutritionCubit`.
4. **Health Passport** — restyle `lib/features/health_passport/`: segmented control, timeline rail, vaccine status chips, weight chart, med-course progress. Keep the existing repositories.
5. **Care calendar** — a new screen grouping reminders into Overdue / This month / Later, with complete-in-place.
6. **Vet Chat** — restyle `lib/features/vet_chat/`. Critical: keep the persistent composer, add the suggestion-chip row above it, add the emergency block driven by `assets/data/triage_rules.json`, and keep the disclaimer readable (`rgba(233,233,237,0.72)`, not dimmer).
7. **Nutrition** — restyle `lib/features/nutrition/`: conic ring (a `CustomPainter` beats `fl_chart` here), champagne treat-budget bar, quick-add chips for the pet's usuals.
8. **Breed Scanner + result** — viewfinder framing, and the post-scan locked "Breed health watchlist" teaser that opens the paywall.
9. **Memory Timeline** — the "Story" masthead, month grouping, full-bleed photo cards.
10. **Paywall + Settings** — both paywall surfaces (bottom sheet for soft teasers, full-screen for hard gates), RevenueCat wiring, and the Settings screen including delete-account with its 30-day grace note. Restore purchases and Terms/Privacy links are mandatory on both paywall surfaces.
